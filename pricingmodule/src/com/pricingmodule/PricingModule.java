...
public class PricingModule {
	...
	// to store the customers entered choice y/N
	char frame_val;
...
//Takes input from user and calculate price 
	public void calculatePrice() {
		...
		System.out.println("Frame : " + pricelist.get(frame)+ " rs");
		
		frame_val = sc.next().charAt(0);
		
		System.out.println("handle bar with brakes : " + pricelist.get(handle_bar_with_brakes)+ " rs");
		
		handle_bar_with_brake_val = sc.next().charAt(0);
		
		System.out.println("seating : " + pricelist.get(seating) + " rs");
		
		seating_val = sc.next().charAt(0);
		
		System.out.println("wheels : " + pricelist.get(wheels)+ " rs");
		
		wheels_val = sc.next().charAt(0);
		
		System.out.println("chain assembly : " + pricelist.get(chain_assembly) + " rs");
